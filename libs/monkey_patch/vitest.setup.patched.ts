...
// the debug monkey patch, then runs the reusable shared suite (libs/shared-tests) against a real
// global-state variant. The point is a regression guarantee — the shared behavior of the state
// libraries must be identical whether or not the debug patch is installed. Set DEBUG_PATCH=off
// to run the same suite WITHOUT the patch and confirm parity.
//
...
// 5) The shared suite is variant-neutral; declare no extra reserved metadata keys (matches the
//    universal / web variants, which inject none). See libs/shared-tests/expectMetadata.ts.
...
//    normal variants keep exact-match semantics. See libs/shared-tests/expectCalledWithStore.ts.
