...
export default defineConfig({
  test: {
    ...
    reporters: ['dot'],
    // The mobile variant runs the neutral suite (test/universal) plus its own async-storage
    // tests (test/native).
    include: ['../test/universal/**/*.{test,spec}.{ts,tsx}', '../test/native/**/*.{test,spec}.{ts,tsx}'],
  },
  resolve: {
    alias: [
