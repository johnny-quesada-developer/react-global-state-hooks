export type {
  ...
  // Base types re-exported for parity with the web package. These are NOT redeclared locally
  // below (the ones this package overrides with async-storage-aware variants — BaseMetadata,
  // StateHook, StoreTools, MetadataSetter, MetadataGetter, ObservableFragment,
  // ActionCollectionConfig, ActionCollectionResult, GlobalStoreCallbacks — are intentionally
  // omitted here and declared further down).
  Any,
  AnyActions,
  BrandedId,
  CleanupFunction,
...
export interface StateHook<State, StateMutator, Metadata extends BaseMetadata> extends StateApi<
  State,
  StateMutator,
  Metadata
> {
