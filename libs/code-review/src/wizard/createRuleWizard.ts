...
export async function createRuleWizard(context: ReviewContext): Promise<string> {
  const { ask, logger, workspaceRoot, rulesDirectory } = context;
  ...
  logger.success(
    `rule saved: ${path.relative(workspaceRoot, file)} → run it with: review <target> --rule ${definition.id}`,
  );
  return file;
}
...
async function writeRuleFile({
  ...
  fs.writeFileSync(file, format === 'json' ? renderJsonRule(definition) : renderTsRule({ definition }));
  return file;
}
...
export function renderTsRule({ definition }: { definition: PromptRuleDefinition }): string {
  return `import { createPromptRule } from '${PACKAGE_NAME}';

// Generated by \`review rule create\`. Edit the definition freely, or replace createPromptRule
