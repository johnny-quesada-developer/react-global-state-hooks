...
import type { NxProject } from '../../shared/workspace';
...
export type Target =
  ...
  | { kind: 'workingChanges' }
  | { kind: 'nxProject'; project: NxProject };
...
export async function detectTarget({
  ...
  const existingPath = [path.resolve(invocationDirectory, value), path.resolve(workspaceRoot, value)].find(
    (candidate) => fs.existsSync(candidate),
  );
  if (existingPath) {
    return fs.statSync(existingPath).isDirectory()
      ? { kind: 'folder', path: existingPath }
      : { kind: 'file', path: existingPath };
  }

  const project = projects.find(({ name }) => name === value);
  if (project) return { kind: 'nxProject', project };

  const looksLikeCommit = COMMIT_PATTERN.test(value) || value === 'HEAD' || value.startsWith('HEAD~');
  if (looksLikeCommit && (await isCommit({ cwd: workspaceRoot, reference: value })))
    return { kind: 'commit', commit: value };
...
export function describeTarget(target: Target, workspaceRoot: string): string {
  switch (target.kind) {
    ...
    case 'nxProject':
      return `nx project ${target.project.name}`;
  }
}
