...
// variant also re-exports from the base package `react-hooks-global-states`, resolved to the
// sibling universal src. Built-artifact / CJS-shape correctness is verified separately by
// `yarn test:interop`.
...
export default defineConfig({
  test: {
    ...
    include: ['../test/universal/**/*.{test,spec}.{ts,tsx}', '../test/web/**/*.{test,spec}.{ts,tsx}'],
  },
  resolve: {
    alias: [
      // Deep subpaths must be listed before the bare barrel so they win for deep imports.
      { find: /^react-global-state-hooks\/(.*)$/, replacement: `${src}/$1` },
      { find: /^react-global-state-hooks$/, replacement: `${src}/index.ts` },
