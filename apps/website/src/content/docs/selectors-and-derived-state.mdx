...
<Callout type="warning" title="Selectors that build new values">
  ...
  counts as a change for that component. Pass `{ isEqual: shallowCompare }` to compare the items. Shared selector hooks can
  reuse the computation across components and accept the same equality option.
</Callout>
