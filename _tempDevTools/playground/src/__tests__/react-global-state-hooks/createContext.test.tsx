...
import { type ContextStoreTools, createContext, InferAPI } from 'react-global-state-hooks';
...
describe('createContext', () => {
  ...
  it('should correctly execute the lifecycle callbacks', async ({ renderHook, strict }) => {
    ...
    // Under the devtools patch, actions/setState are wrapped for logging, so their
    // references differ from context.current. The storeTools *shape* must still match:
    // every base-API member is present (notably `use` is never undefined).
    const lifecycleTools = expect.objectContaining({
    ...
    expect(initSpy).toHaveBeenCalledWith(lifecycleTools);
    expect(onMountedSpy).toHaveBeenCalledWith(lifecycleTools);
