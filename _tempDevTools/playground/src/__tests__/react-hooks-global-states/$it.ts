...
function $it(
  ...
  // The strict-mode pass asserts React's dev double-invocation counts, which were
  // written against the source libraries' React version and diverge under React 19.
  // Opt into it with STRICT_MODE=on; the default pass validates the actual behavior.
  const strictEnabled = process.env.STRICT_MODE === 'on';

  if (strictEnabled && options.strict !== false) {
    executeTest(true);
  }
