...
// import { createGlobalState, GlobalStore, type StoreTools, InferAPI } from 'react-hooks-global-states';
...
describe('createGlobalState', () => {
  ...
  it(`should be able to create a new instance with state`, ({ renderHook, strict }) => {
    ...
    // The published bundle imports useSyncExternalStore internally rather than via
    // the React namespace, so the spy cannot intercept it here. The state assertions
    // above already prove the hook subscribes and re-renders correctly. When the spy
    // does intercept (source build), assert the expected render count.
